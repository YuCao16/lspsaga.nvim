...
act.__index = act
function act.__newindex(t, k, v) rawset(t, k, v) end
...
function act:action_callback()
  ...
  api.nvim_create_autocmd("CursorMoved", {
    ...
    callback = function() self:set_cursor() end,
  })
  api.nvim_create_autocmd({ "WinClosed", "WinLeave" }, {
    buffer = self.action_bufnr,
    callback = function() self:close_action_window() end,
  })
  ...
  if config.code_action.num_shortcut then self:num_shortcut(self.action_bufnr) end
end
...
function act:apply_action_keys()
  map_keys(
    "n",
    config.code_action.keys.exec,
    function() self:do_code_action() end,
    { buffer = self.action_bufnr }
  )
...
function act:get_action_diff(num, main_buf)
  ...
  if not action.edit then return end

  local all_changes = {}
  if action.edit.documentChanges then
    for _, item in pairs(action.edit.documentChanges) do
      if item.textDocument then
        if not all_changes[item.textDocument.uri] then all_changes[item.textDocument.uri] = {} end
  ...
  if not (all_changes and not vim.tbl_isempty(all_changes)) then return end
...
function act:action_preview(main_winid, main_buf)
  ...
  if win_conf.anchor:find("^N") then
    if win_conf.row[false] - opt.height > 0 then
      ...
      if #vim.wo[fn.bufwinid(main_buf)].winbar > 0 then opt.row = opt.row + 1 end
...
function act:clean_context() clean_ctx() end
