...
function libs.get_path_info(buf, level)
  ...
  if level == 1 then return { tbl[#tbl] } end
...
function libs.get_home_dir()
  if libs.is_win then return os.getenv("USERPROFILE") end
  return os.getenv("HOME")
end

function libs.tbl_index(tbl, val)
  for index, v in pairs(tbl) do
    if v == val then return index end
  end
end
...
function libs.has_value(filetypes, val)
  ...
  elseif type(filetypes) == "string" then
    if filetypes == val then return true end
  end
  return false
end
...
function libs.check_lsp_active(silent)
  ...
  if next(active_clients) == nil then
    if not silent then vim.notify("[LspSaga] Current buffer does not have any lsp server") end
    return false
  end
  return true
end
...
function libs.get_lsp_root_dir()
  if not libs.check_lsp_active() then return end
...
function libs.get_config_lsp_filetypes()
  ...
  if next(saga_conf.server_filetype_map) == nil then return filetypes end
...
function libs.find_buffer_by_filetype(ft)
  ...
  for _, bufinfo in pairs(all_bufs) do
    ...
    if type(ft) == "table" and libs.has_value(ft, filetype) then return true, bufinfo["bufnr"] end

    if filetype == ft then return true, bufinfo["bufnr"] end
  end
...
function libs.generate_empty_table(length)
  local empty_tbl = {}
  if length == 0 then return empty_tbl end
...
function libs.add_client_filetypes(client, fts)
  if not client.config.filetypes then client.config.filetypes = fts end
end
...
function libs.scroll_in_preview(bufnr, preview_winid)
  ...
  if preview_winid and api.nvim_win_is_valid(preview_winid) then
    vim.keymap.set("n", config.scroll_preview.scroll_down, function()
      api.nvim_win_call(preview_winid, function() feedkeys("<C-d>") end)
    end, { buffer = bufnr })

    vim.keymap.set("n", config.scroll_preview.scroll_up, function()
      api.nvim_win_call(preview_winid, function() feedkeys("<C-u>") end)
    end, { buffer = bufnr })
  end
end
...
function libs.jump_beacon(bufpos, width)
  if not saga_conf.beacon.enable then return end

  if width == 0 or not width then return end
  ...
  timer:start(
    ...
    vim.schedule_wrap(function()
      ...
      if blend > 100 then blend = 100 end
