...
local function init_kind()
  ...
  local function find_index_by_type(k)
    for index, opts in pairs(kind) do
      if opts[1] == k then return index end
    end
    return nil
  end
  ...
  resolved = function() return kind end
end
...
local function gen_outline_hi(kind)
  for _, v in pairs(kind) do
    ...
    if not ok or not tbl.foreground then api.nvim_set_hl(0, hi_name, { fg = v[3] }) end
  end
end

local function init_kind_hl()
  if not resolved then init_kind() end
  local kind = resolved()
  gen_symbol_winbar_hi(kind)
  gen_outline_hi(kind)
end

local function get_kind() return resolved() end
