...
function def:title_text(opts, link)
  ...
  if #link > 2 then link = table.concat(link, libs.path_sep, #link - 1, #link) end
  opts.title = {
    { link, "TitleString" },
  }
  local data = libs.icon_from_devicon(vim.bo.filetype)
  if data[1] then table.insert(opts.title, 1, { data[1] .. " ", data[2] }) end
end
...
function def:peek_definition()
  ...
  lsp.buf_request_all(current_buf, "textDocument/definition", params, function(results)
    ...
    for _, res in pairs(results) do
      if res and res.result then result = res.result end
    end
    ...
    if fn.has("nvim-0.9") == 1 and config.ui.title then self:title_text(opts, link) end
...
local function unpack_maps()
  ...
  for key, val in pairs(maps) do
    if key ~= "quit" or "close" then res[key] = val end
  end
  return res
end
...
function def:apply_aciton_keys(pos)
  ...
  keymap.set("n", config.definition.quit, function()
    ...
    local node = ctx.data[index]
    if api.nvim_win_is_valid(node.prev) then api.nvim_set_current_win(node.prev) end
    self:close_window(node.winid)
    remove(index)
  end, opt)

  keymap.set("n", config.definition.close, function()
    if vim.tbl_isempty(ctx) then return end
...
def = setmetatable(def, {
  __newindex = function(_, k, v) ctx[k] = v end,
  __index = function(_, k, _) return ctx[k] end,
})
